<# 
Modified: 9/27/2017 By: Solomon Acquah - Includes new export field for Name
.SYNOPSIS 
...
function GenFileName {
    ...
    function TheObject {
        ...
        $defaultProperties = @('FullAccountName', 'FullName', 'Name', 'Parent', 'Depth')
    ...
    function GenFormattedOutput {
        ...
        $outtext = $header
        $outtext += ($Script:AllResults | format-table -AutoSize Hierarchy, FullName, Name, Enabled, Class, @{Label="Members"; expression={$_.Members}; align='right'} | out-string).Trim("`n").TrimStart("`n")
        $outtext += $footer
        if ($picture) {
            DrawIt $outtext
        }
